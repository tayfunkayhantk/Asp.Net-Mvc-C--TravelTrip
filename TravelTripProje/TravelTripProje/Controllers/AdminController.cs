...
namespace TravelTripProje.Controllers
{
    public class AdminController : Controller
    {
        ...
        {
            var m = c.İletisims.ToList();
            return View(m);
        }
        public ActionResult MesajSil(int id)
        {
            var x = c.İletisims.Find(id);
            c.İletisims.Remove(x);
            c.SaveChanges();
            return RedirectToAction("Mesajlar");
        }
        public ActionResult Hakkımızda()
        {
            var Hakkımızda = c.Hakkimizdas.ToList();
            return View(Hakkımızda);
        }
        public ActionResult HakkımızdaGetir(int id)
        {
            var y = c.Hakkimizdas.Find(id);
            return View("HakkımızdaGetir", y);
        }
        public ActionResult HakkımızdaGuncelle(Hakkimizda h)
        {
            var hk = c.Hakkimizdas.Find(h.ID);
            hk.Aciklama = h.Aciklama;
            hk.ResimUrl = h.ResimUrl;
